...
<?php if (!$isEdit && $venues === []): ?>
    <div class="card" style="max-width: 760px; margin: 0 auto;">
        <div class="empty-state">
            <div class="empty-state-title">No approved venues yet</div>
            <div class="empty-state-desc">Courts can be added once the Platform Admin approves one of your venues.</div>
            <a href="<?= url('/owner/venues') ?>" class="btn btn-primary" style="margin-top: 12px;">Go to My Venues</a>
        </div>
    </div>
<?php else: ?>
        ...
        <form id="courtForm" method="POST" action="<?= url($isEdit ? '/owner/courts/' . $courtId : '/owner/courts') ?>" novalidate>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px;">
                <div class="form-group">
                    ...
                    <select name="sport_type_id" id="courtSport" class="form-select<?= $invalid('sport_type_id') ?>" required>
            ...
            <div style="display: flex; justify-content: flex-end; gap: 12px; margin-top: var(--space-6);">
                <a href="<?= url($backUrl) ?>" class="btn btn-secondary">Cancel</a>

<?php
/** @var int $bookingId */
// TODO: load the booking; the refund tier or cash classification is decided by the booking service.
$is_cash = $bookingId === 4;
?>
...
<div class="card" style="max-width: 680px; padding: var(--space-8); margin: 0 auto;">
    ...
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: var(--space-4);">
        ...
        <div>
            ...
            <div class="text-xs text-muted">Booking Reference: <strong>#<?= e($bookingId) ?></strong> (32 hours until start)</div>
        </div>
    </div>
    ...
    <?php if (!$is_cash): ?>
        ...
        <div style="background: #fffbeb; border: 1.5px solid #fde68a; border-radius: var(--radius-lg); padding: 16px; margin-bottom: var(--space-6);">
            ...
            <div style="background: white; padding: 12px; border-radius: var(--radius-md); border: 1px dashed #f59e0b; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
                ...
                <a href="<?= url('/customer/bookings/' . $bookingId) ?>" class="btn btn-primary btn-sm">
    ...
    <form method="POST" action="<?= url('/customer/bookings/' . $bookingId . '/cancel') ?>" onsubmit="return confirm('Cancel this booking? This cannot be undone.');">
        <?= csrf_field() ?>
        ...
        <div style="display: flex; justify-content: flex-end; gap: 12px; margin-top: var(--space-6);">
            <a href="<?= url('/customer/bookings/' . $bookingId) ?>" class="btn btn-secondary">
                Keep My Reservation
            </a>

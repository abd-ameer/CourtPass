<div class="page-header">
 ...
 <div>
 <a href="<?= url('/coach-profile') ?>?id=c1" target="_blank" class="btn btn-outline">
 ...
 <div id="reply-box-101" style="background: var(--color-bg); border-radius: var(--radius-lg); padding: var(--space-4);">
 <div style="font-weight: 700; font-size: 12px; margin-bottom: 6px; color: var(--color-text-heading);">Post Official Coach Reply:</div>
 <textarea class="form-control" id="reply-text-101" rows="2" placeholder="Write your public response to Kasun (e.g. Thanks for the feedback, great progress today!)..." style="font-size: 12px; margin-bottom: 8px;"></textarea>
 <div style="display: flex; justify-content: space-between; align-items: center;">
 <span style="font-size: 11px; color: var(--color-text-subtle);">Only 1 official reply permitted per review.</span>
 <button class="btn btn-sm btn-primary" onclick="submitReply('101', 'Kasun Mendis')">Post Reply</button>
 </div>
 </div>
 </div>
 </div>
...
<script>
function submitReply(reviewId, studentName) {
 const textInput = document.getElementById(`reply-text-${reviewId}`);
 const val = textInput.value.trim();
 if (!val) {
 CourtPassApp.showToast('Please type a reply message before posting.', 'error');
 return;
 }

 const container = document.getElementById(`reply-box-${reviewId}`);
 container.outerHTML = `
 <div style="background: #e6f8f0; border-left: 3px solid var(--color-primary-active); border-radius: var(--radius-md); padding: var(--space-3) var(--space-4); margin-top: var(--space-2);">
 <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
 <div style="font-weight: 700; font-size: 12px; color: var(--color-primary-active);">Coach Dilshan (Official Response)</div>
 <div style="font-size: 10px; color: var(--color-text-subtle);">Just now</div>
 </div>
 <p style="font-size: 12px; color: var(--color-text-body); margin-bottom: 0; line-height: 1.5;">
 "${val}"
 </p>
 ...
 document.getElementById(`review-card-${reviewId}`).style.borderLeft = '';
 CourtPassApp.showToast(`Official reply posted to ${studentName}'s review!`, 'success');
}
</script>

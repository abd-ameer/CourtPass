<div class="page-header">
 ...
 <button class="btn btn-sm btn-outline" style="color: var(--color-danger); border-color: var(--color-danger);" onclick="rejectCoach('Coach Shanilka Fernando', 'coach-card-1')">Reject</button>
 <button class="btn btn-sm btn-primary" onclick="verifyCoach('Coach Shanilka Fernando', 'coach-card-1')"> Grant Verified Badge</button>
 </div>
 </div>
 ...
 <button class="btn btn-xs btn-outline" onclick="CourtPassApp.showToast('Previewing WSF Certificate PDF...', 'info')"> WSF Certificate</button>
 <button class="btn btn-xs btn-outline" onclick="CourtPassApp.showToast('Previewing NIC Copy...', 'info')"> NIC Copy</button>
 </div>
 </div>
 </div>
 ...
 <div style="display: flex; gap: 8px;">
 <button class="btn btn-sm btn-outline" style="color: var(--color-danger); border-color: var(--color-danger);" onclick="rejectCoach('Coach Priyantha Dias', 'coach-card-2')">Reject</button>
 <button class="btn btn-sm btn-primary" onclick="verifyCoach('Coach Priyantha Dias', 'coach-card-2')"> Grant Verified Badge</button>
 </div>
 </div>
 ...
 <span style="font-size: 11px; color: var(--color-text-subtle); text-transform: uppercase; font-weight: 700;">Uploaded Evidence</span>
 <div style="display: flex; gap: 6px; margin-top: 4px; flex-wrap: wrap;">
 <button class="btn btn-xs btn-outline" onclick="CourtPassApp.showToast('Previewing ITTF Certificate PDF...', 'info')"> ITTF Accreditation</button>
 </div>
 </div>
 </div>
...
<script>
function verifyCoach(name, cardId) {
 document.getElementById(cardId).style.display = 'none';
 CourtPassApp.showToast(`Verified Coach badge successfully granted to ${name}!`, 'success');
}

function rejectCoach(name, cardId) {
 const reason = prompt(`Specify reason for rejecting verification of ${name}:`, 'Certificate verification could not be validated with national sports association.');
 if (reason) {
 document.getElementById(cardId).style.display = 'none';
 CourtPassApp.showToast(`Coach verification rejected with notice sent.`, 'error');
 }
}
</script>

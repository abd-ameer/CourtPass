/**
 * CourtPass front-end helpers: fetch wrapper for /api/ routes, form posting with CSRF,
 * and small UI controls (toasts, modals, dropdowns, client-side form checks).
 */
...
const CourtPass = (() => {
    ...
    /** Submit a POST form to an app route (method may be PUT or DELETE). Extra fields are added as hidden inputs. */
    function post(path, fields = {}, method = 'POST') {
...
const CourtPassApp = {
    ...
    showToast: function(type = 'success', title = 'Success', message = '') {
        ...
        toast.innerHTML = `
            ...
            <button type="button" class="modal-close" onclick="this.parentElement.remove()" style="font-size: 16px;">&times;</button>
        `;
    ...
    /** Confirm, then POST to an app route. */
    confirmPost: function(title, message, confirmText, path, method = 'POST') {
        CourtPassApp.confirmDialog(title, message, confirmText, () => CourtPass.post(path, {}, method));
    },

    /** Ask for a mandatory reason in #mandatoryReasonModal, then POST it as "reason". */
    postWithReason: function(title, subtitle, path) {
        const modal = document.getElementById('mandatoryReasonModal');
        const text = document.getElementById('mandatoryReasonText');
        ...
        text.value = '';
        document.getElementById('mandatoryReasonSubmitBtn').onclick = () => {
            ...
            CourtPass.post(path, { reason: reason });
        };

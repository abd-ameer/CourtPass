...
Before designing the schema we checked the proposal, the Developer Guide and the old prototype schema against each other. Nineteen gaps affected the database. Member A decided four of them (one of those also changed the proposal), and the other fifteen were settled in the design, with the reasoning in section 5.

**Decided by the team (Member A, 22 to 23 Sep 2026)**
...
| D4 | Which bookings confirm automatically | A booking paid online is confirmed as soon as PayHere verifies the payment. A cash-on-arrival booking stays Pending until the owner confirms or rejects it. Written into the Revised Proposal and the Codebase Map on 23 Sep 2026 |

**Proposal update made**

| # | Issue | What the proposal said | What changed |
| --- | --- | --- | --- |
| P1 | Auto-confirmation rule | Only "Owners confirm, reject, or cancel bookings" (UC-VO-06) and "verify payment before confirming the related booking". No auto-confirm rule is written anywhere | Done on 23 Sep 2026. The Booking Engine requirements and UC-VO-06 now say that an online-paid booking is confirmed automatically once the payment is verified, and a cash-on-arrival booking waits for the owner. The Codebase Map carries the same wording |
...
- Reliability score formula and the exact penalty per no-show or irresponsible cancellation (Member A). Reliability is not part of the interim, so this is only needed for Increment 3 (Month 6, November).
...
An online booking goes from pending\_payment straight to confirmed when the payment is verified, so it never waits in pending. Only cash-on-arrival bookings wait for the owner. `no_show` can also change to `completed` when an admin upholds a no-show dispute, which removes the penalty on the next recalculation.
